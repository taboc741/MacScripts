...
##  Created by Chris on 5/31/18. Last edited by Chris 4/15/2019
...
if [[ $restartRequired = "none" && $shutDownRequired = "none" ]]; then
    sendToLog "no reboot required, Alerting user Updates complete"
	##Map some variables to pretty up the command to prompt the user
...
if [[ $shutDownRequired != "none" ]]; then
    sendToLog "Shutdown is required.  Issuing system halt. Attempting to save log to Jamf"
    sleep 30 && /sbin/halt & exit 0
else

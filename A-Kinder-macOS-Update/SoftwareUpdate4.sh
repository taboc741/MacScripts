...
updatesNoRestart=`echo $updates | grep --ignore-case recommended | grep --ignore-case -v restart`
[[ -z $updatesNoRestart ]] && updatesNoRestart="none"
restartRequired=`echo $updates | grep --ignore-case restart | grep -v '\*' | cut -d , -f 1`
